...
- **No `-kvfp8` bundle ships.** `--kv-cache-dtype fp8` resolves to a
  ...
  **[Examples → FP8 KV cache](/en/examples/memory-tiers/fp8-kv-cache)**.
...
- **vLLM Docker container** running at `/workspace` (mounts repo
  root). See **[Installation → vLLM setup](/en/getting-started/installation/vllm)**.
