...
unbounded. See **[Model config](/en/reference/model-config)**.
:::
...
**[Reference → Cluster config](/en/reference/cluster-config#runtime-overrides-optional)**.
...
- **EP** runs on the DP dim, plus the TP dim when EP spans past one
  instance's GPUs. The PP dim is never involved — vLLM's EP group pins
  the pipeline stage.
...
| `rtx4090_single_instance.json` | RTX 4090, Llama-3.1-8B TP=1, `mem_util` calibrated | [Validation](../validation) |
| `rtx4090_tp2_instance.json` | 2x RTX 4090 as `tp=2`, Llama-3.1-8B. Needs an RTX4090 `tp=2` profile first — only `tp1` is bundled, so it raises `FileNotFoundError` as shipped | [Profiler](../profiler/overview) |
