...
> **[Profiler → Adding hardware](/en/profiler/adding-hardware)**)
> and rerun.
...
- **[Simulator → KV cache & memory](/en/simulator/scheduling/kv-cache-and-memory)**:
  the `bytes_per_block` formula and how `kv_fp` flows into
  the scheduler's memory check.
- **[Profiler → Output bundle](/en/profiler/output-bundle)**:
