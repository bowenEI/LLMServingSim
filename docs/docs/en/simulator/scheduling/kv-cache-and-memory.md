...
> **[Examples → CXL extended memory](/en/examples/memory-tiers/cxl-memory)**
> for placement rules and
> **[Examples → Prefix caching](/en/examples/memory-tiers/prefix-caching)**
...
derived — see **[Model config](/en/reference/model-config)**. On Qwen3
...
- **Weights placed there explicitly** via the `placement` field in
  ...
  **[Examples → CXL memory](/en/examples/memory-tiers/cxl-memory)**
...
variants: **[Reading the output](/en/simulator/reading-output#heartbeat-block)**.
...
- **[Examples → Prefix caching](/en/examples/memory-tiers/prefix-caching)**
  and **[CXL memory](/en/examples/memory-tiers/cxl-memory)**: the
  configuration angle.
