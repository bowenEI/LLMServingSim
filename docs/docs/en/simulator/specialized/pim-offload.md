...
**[Examples → PIM attention offload](/en/examples/disaggregated/pim-attention-offload)**.
...
[Examples → Sub-batch interleaving](/en/examples/advanced/sub-batch-interleaving).
...
- Widen the node's `cpu_mem.mem_size` to buy more PIM channels — channel
  ...
  **[PIM config](/en/reference/pim-config#how-the-numbers-are-derived)**.
