...
actually got. See
**[Cluster config → Runtime overrides](/en/reference/cluster-config#runtime-overrides-optional)**.
...
definitions; see **[Bench CLI](/en/reference/bench-cli)**.
:::
...
[Workloads → JSONL format](/en/workloads/jsonl-format#why-token-ids-matter)).
...
**[Validation](/en/validation)**; the harness that produces them is
**[Bench CLI](/en/reference/bench-cli)**. Complete real logs for
...
- **[Reference → CLI flags](/en/reference/cli-flags)**: every
  flag that affects the output.
- **[Examples](/en/examples)**: worked configurations to compare
  your output against.
