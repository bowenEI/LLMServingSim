...
does, see the **[Simulator](/en/simulator/architecture)** and
**[Profiler](/en/profiler/overview)** sections.
...
| Intent | Edit |
| --- | --- |
| Add a new hardware target | Run the profiler with `HARDWARE=` set; output lands in `profiler/perf/<hw>/`. See **[Profiler / Adding hardware](/en/profiler/adding-hardware)** |
| Add a new model architecture | Drop a YAML in `profiler/models/<model_type>.yaml`. See **[Profiler / Adding model architecture](/en/profiler/adding-model-architecture)** |
...
**[Reference / Cluster config](/en/reference/cluster-config)**.
...
**[Workloads / ShareGPT generators](/en/workloads/sharegpt-generators)**
