...
- **[Profiler guide](/en/profiler/overview)**: capture per-layer
  CUDA kernel timings into the per-category CSV bundle the simulator
  consumes.
- **[Bench CLI](/en/reference/bench-cli)**: run vLLM end-to-end and
  validate the simulator against ground truth, with every flag for both
  subcommands.
- **[Validation](/en/validation)**: the accuracy numbers those runs
  produced.
