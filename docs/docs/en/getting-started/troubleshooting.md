...
**[Adding a model architecture](/en/profiler/adding-model-architecture)**.

**Fix:** either

- pick a hardware / model / precision combo that's already profiled
  (`ls profiler/perf/`), or
- run the **[Profiler](/en/profiler/overview)** to generate the
...
[Profiler → Running](/en/profiler/running)).
